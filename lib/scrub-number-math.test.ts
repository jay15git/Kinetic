...
describe("normalizeScrubFieldSettings", () => {
  ...
  it("clamps invalid scrub thresholds and wheel sensitivity", () => {
    const normalized = normalizeScrubFieldSettings({
      ...DEFAULT_SCRUB_FIELD_SETTINGS,
      scrub: {
        ...DEFAULT_SCRUB_FIELD_SETTINGS.scrub,
        threshold: 0,
        wheelSensitivity: 999,
      },
    })

    expect(normalized.scrub.threshold).toBe(1)
    expect(normalized.scrub.wheelSensitivity).toBe(200)
  })

  it("rejects fineStep that is not finer than step", () => {
    const normalized = normalizeScrubFieldSettings({
      ...DEFAULT_SCRUB_FIELD_SETTINGS,
      step: 1,
      scrub: {
  ...
  it("resolves duplicate fine and coarse modifiers", () => {
    ...
    expect(normalized.scrub.fineModifier).toBe("alt")
    expect(normalized.scrub.coarseModifier).not.toBe("alt")
  })
})
